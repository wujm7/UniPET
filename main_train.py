...
def main(json_path=''):
    ...
    parser.add_argument('--opt', type=str, default='train_unipet_G1D30.json', help='Path to option JSON file.')
    ...
    for phase, dataset_opt in opt['datasets'].items():
        ...
        if phase == 'train':
            ...
            # 从训练集读取显像剂数量并注入 opt['netG']['num_tracers']
            num_tracers = getattr(train_set, 'num_tracers', 4)
            opt['netG']['num_tracers'] = num_tracers
            if opt['rank'] == 0:
                logger.info(f'Number of tracers detected: {num_tracers}')
                tracer_map = getattr(train_set, 'tracer2idx', {})
                logger.info(f'Tracer-to-index mapping: {tracer_map}')
    ...
    for epoch in range(3000):  # keep running
        ...
        for i, train_data in enumerate(train_loader):
            ...
            if current_step % opt['train']['checkpoint_print'] == 0 and opt['rank'] == 0:
                ...
                if 'G_loss_cls' in logs.keys():
                    logger_tensorboard.add_scalar('TRAIN Generator LOSS/G_loss_cls', logs['G_loss_cls'], global_step=current_step)
                if 'G_loss_orth' in logs.keys():
                    logger_tensorboard.add_scalar('TRAIN Generator LOSS/G_loss_orth', logs['G_loss_orth'], global_step=current_step)
            ...
            if current_step % opt['train']['checkpoint_test'] == 0 and opt['rank'] == 0:
                ...
                for idx, test_data in enumerate(test_loader):
                    with torch.no_grad():
                        ...
                        # print(H_img.shape)
                        # print(E_img.shape)
                        #error1 = torch.abs(E_img-H_img)
                        #errorsum1 = error1 + errorsum1
                        #kE = results['E'].permute(0,2,3,1).contiguous()
                        #kE = torch.fft.fftshift(torch.fft.fft2(torch.view_as_complex(kE),dim=(-2,-1)),dim=(-2,-1))
                        #kH = torch.cat((E_img,torch.zeros_like(E_img)),dim=1)
                        #kH = kH.permute(0,2,3,1).contiguous()
                        #kH = torch.fft.fftshift(torch.fft.fft2(torch.view_as_complex(kH),dim=(-2,-1)),dim=(-2,-1))
                        #error2 = torch.abs(kE-kH)

                        #errorsum2 = error2 + errorsum2

                       # print(torch.sum(error1),torch.sum(error2))
                ...
                # print(torch.sum(errorsum1)/t,torch.sum(errorsum2)/t)
