...
def cartesian_mask(shape: object, acc: object, centred: object = False,
    ...
    elif acc == 8 or acc == 10:
        center_fraction = 0.04
    elif acc == 16 or acc == 20:
...
def calculate_psnr_single(img1, img2, border=0):
    ...
    # img2 = img2.astype(np.float64)
    # gt recon
    return skimage.metrics.peak_signal_noise_ratio(img1, img2,data_range=img2.max())
...
def calculate_ssim_single(img1, img2, border=0):
    ...
    return skimage.metrics.structural_similarity(img1, img2,data_range=img2.max())
